...
with open("Baxter-MANIEA-P-UndirectedEdges-0.8-0.9-1.csv",'r',newline='') as f:
    f_csv = csv.reader(f)
    for item in f_csv:
        uedge.append(item)

with open("Baxter-MANIEA-P-DirectedEdges-0.8-0.9-1.csv",'r',newline='') as f:
    f_csv = csv.reader(f)
    for item in f_csv:
        dedge.append(item)
...
plt.savefig('Baxter-MANIEA-P-NetworkModel-DrawNetworks.png',dpi=500)
plt.show()
...
with open('Baxter-MANIEA-P-IndexforIndividualNodes.csv', 'w', newline='') as f:
    f_csv = csv.writer(f)

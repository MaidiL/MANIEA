...
with open("Baxter-MANIEA-NegativeDirectedEdges-0.8-0.9-0.1.csv",'r',newline='') as f:
    f_csv = csv.reader(f)
    for item in f_csv:
        negDedge.append(item)
with open("Baxter-MANIEA-PositiveDirectedEdges-0.8-0.9-0.1.csv",'r',newline='') as f:
    f_csv = csv.reader(f)
    for item in f_csv:
        posDedge.append(item)
with open("Baxter-MANIEA-UndirectedEdges-0.8-0.9-0.1.csv",'r',newline='') as f:
    f_csv = csv.reader(f)
    for item in f_csv:
        uedge.append(item)
...
nx.draw(Dneg, with_labels=False, node_size=de3, node_color=colorLs3, alpha=1, linewidths=3,
        font_color='black', edge_color='red', width=1, style='dotted',
        pos=position)
plt.savefig('Baxter-MANIEA-NetworkModel-DrawNetworks.png',dpi=500)
plt.show()
...
with open('Baxter-MANIEA-IndexforIndividualNodes.csv', 'w', newline='') as f:
    f_csv = csv.writer(f)

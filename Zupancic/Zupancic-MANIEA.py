...
def rulesGenerator(frequentPatterns, minConf, rules, minLift, lenDataset):
    ...
    for frequentset in frequentPatterns:
        if i % 100 == 0:
            print("Progress:", round(i / len(frequentPatterns), 2))
...
if __name__ == '__main__':
    ...
    print("len(healthOtudata)", len(healthOtudata))
    ...
    plt.savefig('Zupancic-MANIEA-NetworkModel-' + str(minSup) + '-' + str(minConf) + '-' + str(minCor) + '.png',
                dpi=500)
    plt.show()

    with open('Zupancic-MANIEA-AssociationRules-' + str(minSup) + '-' + str(minConf) + '-' + str(minCor) + '.csv', 'w',
              newline='') as f:
        f_csv = csv.writer(f)
    ...
    with open('Zupancic-MANIEA-UndirectedEdges-' + str(minSup) + '-' + str(minConf) + '-' + str(minCor) + '.csv', 'w',
              newline='') as f:
        f_csv = csv.writer(f)
        f_csv.writerows(uedge)

    with open('Zupancic-MANIEA-PositiveDirectedEdges-' + str(minSup) + '-' + str(minConf) + '-' + str(minCor) + '.csv',
              'w', newline='') as f:
        f_csv = csv.writer(f)
        f_csv.writerows(posDedge)

    with open('Zupancic-MANIEA-NegativeDirectedEdges-' + str(minSup) + '-' + str(minConf) + '-' + str(minCor) + '.csv',
              'w', newline='') as f:
        f_csv = csv.writer(f)

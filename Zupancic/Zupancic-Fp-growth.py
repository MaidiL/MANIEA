...
if __name__=='__main__':
    ...
    minSupport = floor(len(dataset) * minSup)
    ...
    nx.draw(D, with_labels=True, node_size=de2, node_color='green', alpha=1, linewidths=3,
            ...
            pos=position)
    time_end3 = time.time()
    plt.savefig('Zupancic-Fpgrowth-NetworkModel'+str(minSup)+'-'+str(minConf)+'-'+str(minLift)+'.png',dpi=500)
    plt.show()

    with open('Zupancic-Fpgrowth-AssociationRules-'+str(minSup)+'-'+str(minConf)+'-'+str(minLift)+'.csv', 'w', newline='') as f:
        f_csv = csv.writer(f)
    ...
    with open('Zupancic-Fpgrowth-UndirectedEdges-' + str(minSup) + '-' + str(minConf)+'-'+str(minLift) + '.csv', 'w', newline='') as f:
        f_csv = csv.writer(f)
        f_csv.writerows(uedge)

    with open('Zupancic-Fpgrowth-DirectedEdges-' + str(minSup) + '-' + str(minConf)+'-'+str(minLift) + '.csv', 'w', newline='') as f:
        f_csv = csv.writer(f)

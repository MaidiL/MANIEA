...
with open("Zupancic-MANIEA-P-UndirectedEdges-0.4-0.6-0.9.csv",'r',newline='') as f:
    f_csv = csv.reader(f)
    for item in f_csv:
        uedge.append(item)

with open("Zupancic-MANIEA-P-DirectedEdges-0.4-0.6-0.9.csv",'r',newline='') as f:
    f_csv = csv.reader(f)
    for item in f_csv:
        dedge.append(item)
...
plt.savefig('Zupancic-MANIEA-P-NetworkModel-DrawNetworks.png',dpi=500)
plt.show()
...
with open('Zupancic-MANIEA-P-IndexforIndividualNodes.csv', 'w', newline='') as f:
    f_csv = csv.writer(f)

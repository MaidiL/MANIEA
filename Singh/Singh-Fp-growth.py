...
if __name__=='__main__':
    ...
    nx.draw(D, with_labels=True, node_size=de2, node_color='green', alpha=1, linewidths=3,
            ...
            pos=position)
    time_end3 = time.time()
    plt.savefig('Singh-Fpgrowth-NetworkModel-' + str(minSup) + '-' + str(minConf) + '-' + str(minLift) + '.png',
                dpi=500)
    plt.show()

    with open('Singh-Fpgrowth-AssociationRules-'+str(minSup)+'-'+str(minConf)+'-'+str(minLift)+'.csv', 'w', newline='') as f:
        f_csv = csv.writer(f)
    ...
    with open('Singh-Fpgrowth-UndirectedEdges-' + str(minSup) + '-' + str(minConf) + '.csv', 'w', newline='') as f:
        f_csv = csv.writer(f)
        f_csv.writerows(uedge)

    with open('Singh-Fpgrowth-DirectedEdges-' + str(minSup) + '-' + str(minConf) + '.csv', 'w', newline='') as f:
        f_csv = csv.writer(f)

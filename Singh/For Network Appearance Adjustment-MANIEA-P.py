...
with open("Singh-MANIEA-P-UndirectedEdges-0.65-0.75-1.csv",'r',newline='') as f:
    f_csv = csv.reader(f)
    for item in f_csv:
        uedge.append(item)

with open("Singh-MANIEA-P-DirectedEdges-0.65-0.75-1.csv",'r',newline='') as f:
    f_csv = csv.reader(f)
    for item in f_csv:
        dedge.append(item)
...
plt.savefig('Singh-MANIEA-P-NetworkModel-DrawNetworks.png',dpi=500)
plt.show()
...
with open('Singh-MANIEA-P-IndexforIndividualNodes.csv', 'w', newline='') as f:
    f_csv = csv.writer(f)

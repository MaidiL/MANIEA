...
if __name__=='__main__':
    ...
    print("len(eddOtudata)", len(eddOtudata))
    ...
    nx.draw(D, with_labels=True, node_size=de2, node_color='green', alpha=1, linewidths=3,
            ...
            pos=position)
    time_end3 = time.time()
    plt.savefig('Singh-Apriori-NetworkModel-' +str(minSup) +'-'+str(minConf)+ '.png', dpi=500)
    plt.show()

    with open('Singh-Apriori-AssociationRules-'+str(minSup)+'-'+str(minConf)+'-'+str(minLift)+'.csv', 'w', newline='') as f:
        f_csv = csv.writer(f)
    ...
    with open('Singh-Apriori-UndirectedEdges-' + str(minSup) + '-' + str(minConf) + '.csv', 'w', newline='') as f:
        f_csv = csv.writer(f)
        f_csv.writerows(uedge)

    with open('Singh-Apriori-DirectedEdges-' + str(minSup) + '-' + str(minConf) + '.csv', 'w', newline='') as f:
        f_csv = csv.writer(f)

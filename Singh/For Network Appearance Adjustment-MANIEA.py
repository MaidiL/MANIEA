...
with open("Singh-MANIEA-NegativeDirectedEdges-0.65-0.75-0.1.csv",'r',newline='') as f:
    f_csv = csv.reader(f)
    for item in f_csv:
        negDedge.append(item)
with open("Singh-MANIEA-PositiveDirectedEdges-0.65-0.75-0.1.csv",'r',newline='') as f:
    f_csv = csv.reader(f)
    for item in f_csv:
        posDedge.append(item)
with open("Singh-MANIEA-UndirectedEdges-0.65-0.75-0.1.csv",'r',newline='') as f:
    f_csv = csv.reader(f)
    for item in f_csv:
        uedge.append(item)
...
nx.draw(Dpos, with_labels=False, node_size=de2, node_color=colorLs2, alpha=1, linewidths=3,
        font_color='black', edge_color='cornflowerblue', width=1, style='solid',
        pos=position)
nx.draw(Dneg, with_labels=False, node_size=de2, node_color=colorLs3, alpha=1, linewidths=3,
        font_color='black', edge_color='orangered', width=1, style='dotted',
        pos=position)
plt.savefig('Singh-MANIEA-NetworkModel-DrawNetworks.png',dpi=500)
plt.show()
...
with open('Singh-MANIEA-IndexforIndividualNodes.csv', 'w', newline='') as f:
    f_csv = csv.writer(f)

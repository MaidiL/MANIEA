...
with open("Singh-Spearman-PositiveEdges-0.61.csv",'r',newline='') as f:
    f_csv = csv.reader(f)
    for item in f_csv:
        posuedge.append(item)

with open("Singh-Spearman-NegativeEdges-0.61.csv",'r',newline='') as f:
    f_csv = csv.reader(f)
    for item in f_csv:
...
position = nx.spring_layout(U,k=1.5)
...
plt.savefig('Singh-Spearman-NetworkModel-DrawNetworks.png',dpi=500)
plt.show()

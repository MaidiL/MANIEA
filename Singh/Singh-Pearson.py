...
if __name__=='__main__':
    ...
    plt.savefig('Singh-Pearson-NetworkModel-' + str(minCor) + '.png', dpi=500)
    plt.show()
    time_end2 = time.time()

    with open('Singh-Pearson-AssociationRules-'+str(minCor)+'.csv', 'w', newline='') as f:
        f_csv = csv.writer(f)
    ...
    with open('Singh-Pearson-PositiveEdges-' + str(minCor) + '.csv', 'w', newline='') as f:
        f_csv = csv.writer(f)
        f_csv.writerows(posuedge)
    with open('Singh-Pearson-NegativeEdges-' + str(minCor) + '.csv', 'w', newline='') as f:
        f_csv = csv.writer(f)
